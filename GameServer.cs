...
namespace Server.Game
{
    public class GameServer
    {
        ...
        private async Task SendGoToZone_V2(RRConnection conn, string zoneName)
        {
            ...
            // ============================================================================
            // STEP 3: Create PathManager Component (Channel 7, Message 13)
            // CRITICAL FIX: Must create PathManager BEFORE interval messages
            // This creates the PathManager component that the client expects
            // ============================================================================
            await SendCE_PathManager_Create(conn);
               ...
               // await SendCE_Interval_A(conn);
                ...
                // Note: 7/385 (Now connected) is sent AFTER entity spawn in HandleZoneJoin
                // The client will now send Zone Join (13/6), which triggers HandleZoneJoin
        ...
        private async Task HandleZoneJoin(RRConnection conn)
        {
            ...
            // ============================================================================
            // STEP 1: Send Zone Ready (Channel 13, Message 1)
            // Tells the client which zone to load and provides minimap exploration data
            // ============================================================================
            var zoneReady = new LEWriter();
            zoneReady.WriteByte(13);           // Zone channel
            zoneReady.WriteByte(1);            // Zone ready message
            zoneReady.WriteUInt32(1);          // Zone/World ID
            zoneReady.WriteUInt16(0x12);       // Minimap explored bit count (18 tiles)

            // Write minimap exploration data (0xFFFFFFFF = fully explored)
            ...
            // ============================================================================
            // STEP 2: Send Zone Instance Count (Channel 13, Message 5)
            // Provides instance/difficulty information for the zone
            // ============================================================================
            var instanceCount = new LEWriter();
            instanceCount.WriteByte(13);       // Zone channel
            instanceCount.WriteByte(5);        // Instance count message
            instanceCount.WriteUInt32(1);      // Instance value 1
            instanceCount.WriteUInt32(1);      // Instance value 2
            await SendCompressedEResponseWithDump(conn, instanceCount.ToArray(), "zone_instance_count_13_5");

            // ============================================================================
            // STEP 4: Send Interval Message (Channel 7, Message 13)
            // CRITICAL FIX: Must be sent BEFORE entity spawn (GO server does it this way)
            // Sets up the PathManager budget and tick timing for the client
            // Without this before entities, client gets confused and rejects entity data
            // ============================================================================
            ...
            // ============================================================================
            // STEP 5: Spawn Player Entity
            // Creates the player's character entity and all its components in the world
            // Must come AFTER Interval message so PathManager is configured first
            // ============================================================================
            await SendPlayerEntitySpawn(conn);
            await Task.Delay(50);  // Small delay to ensure message processing order

            // ============================================================================
            // STEP 5: Send "Now Connected" (Channel 7, Message 385)
            // Signals to the client that the avatar is ready and connected
            // ============================================================================
            ...
            var nowConnected = new LEWriter();
            nowConnected.WriteByte(7);         // Client entity channel
            nowConnected.WriteUInt16(385);     // "Now connected" message type
            uint avatarId = (avatar != null) ? avatar.ID : 0u;
            nowConnected.WriteUInt32(avatarId); // Avatar entity ID
            await SendCompressedAResponseWithDump(conn, nowConnected.ToArray(), "dfc_7_385_now_connected");
            await Task.Delay(50);

            // ============================================================================
            // STEP 6: Send Initial Position Update (Channel 7, Message 100)
            // Sets the avatar's initial spawn position in the world (town center)
            // ============================================================================
            const ushort AVATAR_ENTITY_ID = 0x0001;
            var posUpdate = new LEWriter();
            posUpdate.WriteByte(7);            // Client entity channel
            posUpdate.WriteByte(0x64);         // Position update opcode (100 decimal)
            posUpdate.WriteByte((byte)((AVATAR_ENTITY_ID >> 8) & 0xFF));  // Entity ID high byte
            posUpdate.WriteByte((byte)(AVATAR_ENTITY_ID & 0xFF));         // Entity ID low byte
            posUpdate.WriteFloat(2400.0f);     // Spawn position X (town center)
            posUpdate.WriteFloat(2400.0f);     // Spawn position Y (town center)
            posUpdate.WriteFloat(0.0f);        // Spawn position Z (ground level)
            await SendCompressedAResponseWithDump(conn, posUpdate.ToArray(), "position_update_7_100");
            await Task.Delay(50);

            // ============================================================================
            // STEP 7: Enable Client Control (Channel 7, Message 53)
            // Grants the client control over their avatar so they can move
            // ============================================================================
            await SendFollowClient(conn);

            Debug.Log($"[Game] HandleZoneJoin: ✅ Zone join sequence complete");

            // ============================================================================
            // STEP 8: Start Periodic Update Loop
            // Continuously sends position and component updates to keep client active
            // Without this, the client will timeout and disconnect after ~10 seconds
            // This runs in a background task and continues until connection closes
            // ============================================================================
            _ = Task.Run(async () =>
            {
                int updateCount = 0;
                try
                {
                    ...
                    {
                        await Task.Delay(1000);  // Send updates every 1 second
                        updateCount++;

                        // Send periodic position update to keep client synchronized
                        var periodicPosUpdate = new LEWriter();
                        periodicPosUpdate.WriteByte(7);      // Client entity channel
                        periodicPosUpdate.WriteByte(0x64);   // Position update opcode
                        periodicPosUpdate.WriteByte(0x00);   // Entity ID high byte
                        periodicPosUpdate.WriteByte(0x01);   // Entity ID low byte (Avatar = 0x0001)
                        periodicPosUpdate.WriteFloat(2400.0f); // Position X
                        periodicPosUpdate.WriteFloat(2400.0f); // Position Y
                        periodicPosUpdate.WriteFloat(0.0f);    // Position Z
                        await SendCompressedAResponseWithDump(conn, periodicPosUpdate.ToArray(), "periodic_pos");

                        // Every 3 seconds, send a component update to keep components alive//CLAUDE AI
                        if (updateCount % 3 == 0)
                        {
                            var componentUpdate = new LEWriter();
                            componentUpdate.WriteByte(7);      // Client entity channel
                            componentUpdate.WriteByte(0x35);   // Component update opcode
                            componentUpdate.WriteByte(0x00);   // Component ID high byte
                            componentUpdate.WriteByte(0x56);   // Component ID low byte (UnitBehavior)
                            componentUpdate.WriteByte(0x64);   // Update type
                            componentUpdate.WriteByte(0x01);   // Update data (enable client control)
                            await SendCompressedAResponseWithDump(conn, componentUpdate.ToArray(), "component_update");
                        }
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogWarning($"[Game] Periodic update loop ended: {ex.Message}");
                }
            });
        }
        ...
        private async Task SendPlayerEntitySpawnGO(RRConnection conn)
        {
            ...
            Debug.Log($"[Game] SendPlayerEntitySpawnGO: Player ID={player.ID}, Avatar ID={avatar.ID}");

            var writer = new LEWriter();
            writer.WriteByte(7);

            // 1. Create AVATAR first (GO order)
            writer.WriteByte(0x01);
            writer.WriteByte((byte)((avatar.ID >> 8) & 0xFF));
            writer.WriteByte((byte)(avatar.ID & 0xFF));
            writer.WriteByte(0xFF);
            writer.WriteCString(avatar.GCClass);
            Debug.Log($"[Game] Created Avatar first: ID={avatar.ID}");

            // 2. Create PLAYER second (GO order)
            writer.WriteByte(0x01);
            writer.WriteByte((byte)((player.ID >> 8) & 0xFF));
            writer.WriteByte((byte)(player.ID & 0xFF));
            writer.WriteByte(0xFF);
            writer.WriteCString(player.GCClass);
            Debug.Log($"[Game] Created Player second: ID={player.ID}");

            // 3. Init Player (GO order)
            ...
            // 4. Create components in GO order: Manipulators, Equipment, QuestManager, DialogManager, UnitContainer, Modifiers, Skills, UnitBehavior
            ...
            foreach (var compName in componentOrder)
            {
                ...
                // Component init data
                if (compName == "UnitContainer")
                {
                    writer.WriteUInt32(1);
                    writer.WriteUInt32(1);
                    writer.WriteByte((byte)component.Children.Count);
                    foreach (var inv in component.Children)
                    {
                        writer.WriteByte(0xFF);
                        writer.WriteCString(inv.GCClass);
                        writer.WriteByte(0x01);
                    }
                    writer.WriteByte(0x00);
                }
                ...
                {
                    // CRITICAL: Write UnitBehavior init with control flags
                    writer.WriteByte(0x01); // IsUnderClientControl = true
                    writer.WriteByte(0x01); // IsOwnedByCurrentPlayer = true
                }
                else
                {
                    writer.WriteByte(0x00);
                ...
                Debug.Log($"[Game] Created component: {compName}");
            }

            // 5. Init Avatar LAST (GO order)
            writer.WriteByte(0x02);
            writer.WriteByte((byte)((avatar.ID >> 8) & 0xFF));
            writer.WriteByte((byte)(avatar.ID & 0xFF));
        ...
        // Mirrors the Go writer: BeginStream -> opcode -> payload [-> EndStream] per frame
           ...
           {
               // PathManager component creation - creates the PathManager component first
               // This must be sent BEFORE interval messages so component exists
               // Based on GO server analysis: PathManager::ReadBudget appears after component creation
               var body = new LEWriter();
               ...
               // PathManager initialization data (matches GO server budget values)
        ...
        private async Task SendCE_Interval_A(RRConnection conn)
        {
            ...
            writer.WriteInt32(1);     // Current server tick
            writer.WriteInt32(100);   // Tick interval
            writer.WriteInt32(0);     // Movement prediction buffer

            // PathManager::readBudget
            writer.WriteInt32(0);     // Unk - CHANGED from -1 to match GO
        ...
        /*private async Task SendPathManagerBudget_A(RRConnection conn)
        {
            var writer = new LEWriter();
            writer.WriteByte(7);     // ClientEntity channel
            writer.WriteByte(0x01);  // PathManager budget opcode

            writer.WriteByte(0x02);  // Synch flag (0x02 for dungeon sync)
            writer.WriteUInt16(100); // PerUpdate budget
            writer.WriteUInt16(20);  // PerPath budget
            writer.WriteUInt16(100); // NoSync PerUpdate
            writer.WriteUInt16(20);  // NoSync PerPath

            writer.WriteByte(0x06);  // End stream

            await SendCompressedAResponseWithDump(conn, writer.ToArray(), "ce_pathbudget_7_01");
        }*/
        ...
        private async Task SendPlayerEntitySpawn(RRConnection conn)
        {
            ...
            try
            {
                ...
                if (!_selectedCharacter.TryGetValue(conn.LoginName, out var character))
                {
                    Debug.LogError($"[Game] No character found for {conn.LoginName}");
                    return;
                }

                var player = character; // This is the Player GCObject
                var avatar = character.Children.FirstOrDefault(c => c.NativeClass == "Avatar");

                if (avatar == null)
                {
                    Debug.LogError($"[Game] No avatar found for character");
                    return;
                }

                // Create the DFC serialized entity data
                var entityWriter = new LEWriter();

                // Write Player
                player.WriteFullGCObject(entityWriter);

                // Write Avatar  
                avatar.WriteFullGCObject(entityWriter);

                // Wrap in channel 7 message
                var writer = new LEWriter();
                writer.WriteByte(7); // ClientEntity channel
                writer.WriteBytes(entityWriter.ToArray());
        ...
        private void CreatePlayerEntity(RRConnection conn, LEWriter writer)
        {
            ...
            // 12. Init Avatar (opcode 0x02) with actual character data
            WriteInitEntity(writer, AVATAR_ID);
            Debug.Log($"[Game] CreatePlayerEntity: Writing Avatar init data");

            // Get the selected character's avatar data
            Server.Game.GCObject avatarObj = null;
            if (_selectedCharacter.TryGetValue(conn.LoginName, out var character))
            {
                avatarObj = character?.Children?.FirstOrDefault(c => c.NativeClass == "Avatar");
                Debug.Log($"[Game] CreatePlayerEntity: Found character for {conn.LoginName}, avatar={(avatarObj != null ? "found" : "null")}");
            }
            else
            {
                Debug.LogWarning($"[Game] CreatePlayerEntity: No selected character found for {conn.LoginName}");
            }

            if (avatarObj != null)
            {
                Debug.Log($"[Game] CreatePlayerEntity: Writing avatar appearance data");
                // Write actual avatar init data from the character object
                writer.WriteByte(0x01);  // Has appearance data
                                         // Write minimal valid data for character class/gender
                writer.WriteByte(0x00);  // Class ID (0 = Fighter)
                writer.WriteByte(0x00);  // Gender (0 = Male)
            }
            else
            {
                Debug.LogWarning($"[Game] CreatePlayerEntity: No avatar object, writing minimal init data");
                writer.WriteByte(0x00);  // No appearance data
            }
